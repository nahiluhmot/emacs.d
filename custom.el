(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(custom-safe-themes
   '("5c8a1b64431e03387348270f50470f64e28dfae0084d33108c33a81c1e126ad6"
     "2a73ef80e6aa1efe7759c4fc1c7e52b6c092976788a26a292365a960f7c8d9b7"
     "2b3f1e6abe0f02ff73d95dca04901bdbc2ecebe80fa453eded34fa39c8b050cb"
     "3a9f65e0004068ecf4cf31f4e68ba49af56993c20258f3a49e06638c825fbfb6"
     "7e377879cbd60c66b88e51fad480b3ab18d60847f31c435f15f5df18bdb18184"
     "e1f4f0158cd5a01a9d96f1f7cdcca8d6724d7d33267623cc433fe1c196848554"
     "4b0e826f58b39e2ce2829fab8ca999bcdc076dec35187bf4e9a4b938cb5771dc"
     "fe2539ccf78f28c519541e37dc77115c6c7c2efcec18b970b16e4a4d2cd9891d"
     "7a994c16aa550678846e82edc8c9d6a7d39cc6564baaaacc305a3fdc0bd8725f"
     "e1ef2d5b8091f4953fe17b4ca3dd143d476c106e221d92ded38614266cea3c8b"
     "1d50bd38eed63d8de5fcfce37c4bb2f660a02d3dff9cbfd807a309db671ff1af"
     "0fe9f7a04e7a00ad99ecacc875c8ccb4153204e29d3e57e9669691e6ed8340ce"
     "fe76f3d5094967034192f6a505085db8db6deb0e135749d9a54dc488d6d3ee2f"
     "4ea0aa360264ff861fb0212abe4161b83ad1d8c8b74d8a04bcd1baf0ebdceeae"
     "427fa665823299f8258d8e27c80a1481edbb8f5463a6fb2665261e9076626710"
     "80365dd15f97396bdc38490390c23337063c8965c4556b8f50937e63b5e9a65c"
     "6b289bab28a7e511f9c54496be647dc60f5bd8f9917c9495978762b99d8c96a0"
     "8aca557e9a17174d8f847fb02870cb2bb67f3b6e808e46c0e54a44e3e18e1020"
     "6b2636879127bf6124ce541b1b2824800afc49c6ccd65439d6eb987dbf200c36"
     "fe666e5ac37c2dfcf80074e88b9252c71a22b6f5d2f566df9a7aa4f9bea55ef8"
     "356e5cbe0874b444263f3e1f9fffd4ae4c82c1b07fe085ba26e2a6d332db34dd"
     "a866134130e4393c0cad0b4f1a5b0dd580584d9cf921617eee3fd54b6f09ac37"
     "fa2b58bb98b62c3b8cf3b6f02f058ef7827a8e497125de0254f56e373abee088"
     "bffa9739ce0752a37d9b1eee78fc00ba159748f50dc328af4be661484848e476"
     default))
 '(elm-format-on-save t)
 '(haskell-stylish-on-save t)
 '(package-selected-packages
   '(0blayout 0x0 0xc 2048-game 2bit a aas abc-mode abl-mode abridge-diff
              ac-html-bootstrap ac-slime afternoon-theme coffee-mode
              company company-go counsel-projectile csv-mode diff-hl
              doom-themes eglot elm-mode evil evil-surround
              exec-path-from-shell flycheck flycheck-rust go-mode
              golden-ratio graphql graphql-mode haskell-mode helm
              helm-ag helm-projectile helm-rg ivy js2-mode lsp-haskell
              lsp-mode lsp-ui magit markdown-mode prettier prettier-js
              projectile projectile-rails rainbow-delimiters rbenv
              rust-mode s smartparens spacemacs-theme sqlformat tide
              typescript-mode ujelly-theme use-package web-mode
              yaml-mode))
 '(safe-local-variable-values
   '((flycheck-clang-include-path ("./include"))
     (flycheck-clang-include-path '("./include"))
     (flycheck-clang-include-path
      (list (expand-file-name "../include")))
     (setq flycheck-clang-include-path
           (list (expand-file-name "../include")))
     (message projectile-projcet-root) (message "testing..."))))
(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 )
